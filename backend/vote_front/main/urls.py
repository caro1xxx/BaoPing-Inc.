from django.conf.urls import url, re_path
from main.views.support import Support
from main.views.voteuser import VoteUser, Comment, RecentVoteRecord, Feedback
from main.views.Request404 import Request404
from main.views.vote_target import VoteTarget
from main.views.vote_target.RecentVoteRecord import RecentFiveVoteRecord
from main.views.vote_activity import AloneVoteActivity, TodayStar
from main.views.keys import Keys
from main.views.gift import Gift
from vote_front import settings
from main.views.Settings import Settings
from django.views.static import serve

urlpatterns = [
    re_path(r'^media/(?P<path>.*)$',serve,{
        'document_root':settings.MEDIA_ROOT
    }),
    url(r'^api/support/$', Support.Support.as_view()),
    url(r'^api/voteuser/$', VoteUser.VoteUser.as_view()),
    url(r'^api/recentvoterecord/$', RecentVoteRecord.RecentVoteRecord.as_view()),
    url(r'^api/votetarget/$', VoteTarget.VoteTarget.as_view()),
    url(r'^api/alonevoteactivity/$', AloneVoteActivity.AloneVoteActivity.as_view()),
    url(r'^api/keys/$', Keys.Keys.as_view()),
    url(r'^api/settings/$', Settings.as_view()),
    url(r'^api/comment/$', Comment.Comment.as_view()),
    url(r'^api/feedback/$', Feedback.Feedback.as_view()),
    url(r'^api/gift/$', Gift.Gift.as_view()),
    url(r'^api/recentfivevoterecord/$', RecentFiveVoteRecord.as_view()),
    url(r'^api/todaystar/$', TodayStar.TodayStar.as_view()),
    url(r'$', Request404.as_view())
]